...
def create_app(config_name=None):
    ...
    # Fix SQLite path on Windows: ensure absolute path with forward slashes
    db_uri = app.config['SQLALCHEMY_DATABASE_URI']
    if db_uri.startswith('sqlite:///'):
        path_part = db_uri[10:]
        # If relative (no drive letter, not already absolute on Unix)
        if ':' not in path_part and not path_part.startswith('/'):
            abs_path = Path(os.path.dirname(os.path.abspath(__file__))) / path_part
            abs_path.parent.mkdir(parents=True, exist_ok=True)
            app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{abs_path.as_posix()}'
