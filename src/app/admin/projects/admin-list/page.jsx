...
export default function AdminProjects() {
  ...
  useEffect(() => {
    fetch('/api/admin/projects/admin-list').then(r => r.json()).then(d => setProjects(d.projects || []));
  }, []);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Projects created by administrators</p>
        </div>
            ...
            <tbody>
              {filtered.map((p, i) => (
                <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-6 py-3 text-gray-500 text-xs">{p.created_at?.slice(0, 10)}</td>
                  <td className="px-6 py-3">
                    <Link href={`/admin/projects/admin-list/${p.id}`} className="p-1.5 hover:bg-blue-50 rounded-lg transition-colors inline-block">
                      <Eye size={15} className="text-blue-500" />
                    </Link>
                  </td>
                </motion.tr>
              ))}
              {!filtered.length && <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-400">No admin projects found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
